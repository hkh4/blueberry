...
type Rhythm = RhythmNumber * int          //int is the number of dots
...
let pdigit2 = pdigit |>> (fun a -> System.Char.GetNumericValue a |> int) <!> "pdigit2"
...
let rhythm = pseq (x64 <|> x32 <|> x16 <|> x8 <|> x4 <|> x2 <|> x1) dot (fun (a,b) -> (a,b)) |>> Rhythm <!> "Rhythm"
...
let eval ast =
   ...
   printfn "%A" o
   printfn "%A" m
   match (evalOption o optionsR) with
   | Some(r) ->
      printfn "%A" r
      Some(r)
   | None ->
